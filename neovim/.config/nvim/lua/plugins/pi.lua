return {
  -- Pi Coding Agent integration — minimalist terminal-based AI assistant
  -- Uses Snacks.nvim terminal (already installed as claudecode.nvim dependency).
  -- No dedicated Neovim plugin exists for pi, so we build a terminal wrapper
  -- modeled after the Claude Code integration.
  --
  -- Three integration approaches exist:
  --   1. Terminal wrapper (this file) — simple, works today
  --   2. RPC mode — spawn `pi --mode rpc` and talk JSON protocol (see docs/rpc.md)
  --   3. SDK — use @earendil-works/pi-coding-agent in a Node.js-hosted plugin
  --
  -- We use approach 1 for now. It gives you:
  --   - Floating terminal (95% width/height, winblend 30, rounded border)
  --   - Toggle with <M-p> or <leader>api
  --   - Focus with <leader>apc
  --   - Send selection with <leader>aps (visual mode)
  --   - Compose prompt in scratch buffer with <leader>app (Ctrl+S to send)
  --   - Kill terminal with <leader>apk, restart with <leader>apR
  {
    "pi-neovim",
    -- Virtual plugin — no actual repo, just sets up keymaps on VeryLazy
    dir = vim.fn.stdpath("config") .. "/lua/plugins",
    event = "VeryLazy",
    config = function()
      -- ── Pi Terminal Management ─────────────────────────────────────
      local pi_buf = nil
      ...
      local function open_pi_terminal()
        -- Use Anthropic (API key via ANTHROPIC_API_KEY) as default.
        -- To use local Ollama instead: "pi --provider ollama --model qwen3.5"
        -- To use Kimi: "pi --provider kimi --model kimi-k2.5"
        Snacks.terminal("pi", {
          name = "pi-agent",
          win = {
            position = "float",
    ...
    keys = {
      { "<M-p>", desc = "Toggle Pi Coding Agent", mode = { "n", "i", "t" } },
    },
  },
}
