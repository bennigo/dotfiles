...
def _cell_size():
    ...
    try:
        import fcntl
        import termios
        packed = fcntl.ioctl(sys.stdout, termios.TIOCGWINSZ,
...
def _draw(self, path, start_x, start_y, width, height):
    ...
    with warnings.catch_warnings(record=True):
        ...
        if image.width > box[0] or image.height > box[1]:
            ...
            image = image.resize(
                (max(1, int(scale * image.width)),
                 max(1, int(scale * image.height))),
    ...
    _log('draw %s\n     cells=%sx%s at (%s,%s) cell_px=%sx%s '
         ...
         % (path, width, height, start_x, start_y,
            self.pix_row, self.pix_col, box[0], box[1],
            image.width, image.height, len(payload) / 1024.0))
